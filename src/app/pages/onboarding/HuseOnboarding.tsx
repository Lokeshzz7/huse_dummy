  ...
  useEffect(() => {
    if (!isLoading) {
      if (!user) {
        ...
        // Find correct step
        if (!user.college_id) setCurrentStep(1);
        else if (!user.phone_verified) setCurrentStep(2);
        else if (!user.subscription_expiry_date && user.subscription_status !== 'active') setCurrentStep(3);
        else if (!user.avatar_style) setCurrentStep(4);
        else {
          // Awaiting admin verification
          navigate('/verification-hub'); // or a "Pending Approval" page
        }
      }
    }
  ...
  return (
    <div className="min-h-screen bg-[#050505] relative flex flex-col items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-3xl mx-auto">
        ...
        {/* Form Container */}
        <div className="bg-[#0F0F0F] border border-purple-500/20 rounded-[25px] p-6 md:p-10 shadow-2xl relative overflow-hidden">
          <AnimatePresence mode="wait">
            ...
            {currentStep === 3 && <Step3DocumentPayment key="step3" onNext={() => setCurrentStep(4)} />}
            {currentStep === 4 && <Step4AvatarSkills key="step4" onComplete={() => navigate('/verification-hub')} />}
          </AnimatePresence>
        </div>
      </div>
    </div>
